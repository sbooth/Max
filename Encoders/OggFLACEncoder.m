...
- (oneway void) encodeToFile:(NSString *) filename
{
	...
	@try {
		...
		if(NULL == _flac) {
			@throw [FLACException exceptionWithReason:NSLocalizedStringFromTable(@"Unable to create the Ogg FLAC encoder.", @"Exceptions", @"") userInfo:nil];
		}
...
- (void) encodeChunk:(const AudioBufferList *)chunk frameCount:(UInt32)frameCount
{
	...
	@try {
		...
		// Encode the chunk
		flacResult = OggFLAC__file_encoder_process(_flac, (const FLAC__int32 * const *)buffer, frameCount);
