...
static struct Version version = { 0U, 7U, 6U, releaseType_release, 0U, };
...
- (void) dispatchNotificationWithDictionary:(NSDictionary *)dict {
	...
	if (![preferences boolForKey:GrowlSquelchModeKey]) {
		/* Look for a displayPlugin as follows:
		 *		First, look at the GrowlApplicationNotification's specified displayPlugin (Growl prefs, per-notification level).
		 *		If there isn't one, see if notification posted by the originating application requested a specific plugin (application-requested).
		 *		Next, look at the ticket's displayPlugin (Growl prefs, per-application level)
		 *		Finally, use the Growl-global default notification (Growl prefs, global)
		 */
...
#pragma unused(theApplication)
	...
	if ([pathExtension isEqualToString:@"growlView"] || [pathExtension isEqualToString:@"growlStyle"]) {
		...
		if ([filename isSubpathOf:NSTemporaryDirectory()]) //assume we got here from GAB
			...
			NSLog(@"%@ is not a subpath of %@",[filename stringByStandardizingPath], [NSTemporaryDirectory() stringByStandardizingPath]);
